class BSTnode:
    ...
    def insert(self, value):
        if not self.data: #SE N TIVER RAIZ
            self.data = value #RAIZ VAI RECEBER O VALOR DO PARAMETRO
        elif value < self.data: # SE O VALOR MENOR QUE A RAIZ
            # VAI PARA A ESQUERDA CARALHO
            if self.left: # SE TIVER DADO NA ESQUERDA
                self.left.insert(value) # CHAMA RECURSIVIDADE 
            else: # SE ESQUERDA FOR NONE
                self.left = BSTnode(value) #VAI ADICIONAR O VALOR NA ESQUERDA - CLASSE
        elif value > self.data: # SE O VALOR FOR MAIOR QUE A RAIZ
            if self.right: #VERIFICAR SE TEM DADO NA DIREITA
                self.right.insert(value) # CHAMA
            else: # SE NÃO TIVER NADA NA DIREITA
                self.right = BSTnode(value) # VAI ADICIONAR O VALOR NA DIREITA - CLASSE
